...
export function updateTokenStatistics(
...
export async function logStatistics(statistics: Statistics | undefined) {
  if (!statistics) {
...
${Object.entries(statistics.clusters)
  .map(([cluster, count]) => `- ${cluster}: ${count.posts}(${count.news})`)
  .join("\n")}`,
        },
      },
    });
...
export function archiveStatistics() {
  ...
  try {
    renameSync(
      statisticsFile,
      path.join(
        path.dirname(statisticsFile),
        `statistics_${date.getDate()}-${date.getMonth()}-${date.getFullYear()}.json`
      )
    );
  } catch (error) {
